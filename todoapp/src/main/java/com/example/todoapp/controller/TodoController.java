...
public class TodoController {
    ...
    private void addTodo() {
        ...
        int id = service.getAllTodos().size() + 1;
        service.addTodo(title, desc, endTime, priority, id);
    ...
    private void viewTodosSortedByPriority() {
        List<Todo> todos = service.getTodosSortedByPriority();
        if (todos.isEmpty()) {
            System.out.println("No todos found.");
        } else {
    ...
    private void updateTodo() {
        ...
        Optional<Todo> opt = service.getTodoById(id);
        if (opt.isPresent()) {
            String title;
            do {
